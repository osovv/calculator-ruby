...
class TestLexer < Test::Unit::TestCase
  def test_addition
    ...
    assert_equal(0.0, calculator.run('(-1) + 1'))
    assert_equal(0.0, calculator.run('0 + 0'))
    assert_equal(2.0, calculator.run('1 + 1'))
  ...
  def test_substraction
    calculator = Calculator.new
    assert_equal(0.0, calculator.run('0 - 0'))
    assert_equal(1.0, calculator.run('3 - 2'))
    assert_equal(10.5, calculator.run('10.7 - 0.2'))
    assert_equal(-15.0, calculator.run('(-8) - 7'))
  end

  def test_multiplication
    calculator = Calculator.new
    assert_equal(0.0, calculator.run('0 * 0'))
    assert_equal(0.0, calculator.run('0 * 123'))
    assert_equal(25.0, calculator.run('5 * 5'))
    assert_equal(12.4, calculator.run('4 * 3.1'))
    assert_equal(-18.0, calculator.run('(-3) * 6'))
  end

  def test_division
    calculator = Calculator.new
    assert_equal(0.0, calculator.run('0 / 1'))
    assert_equal(20.0, calculator.run('100 / 5'))
    assert_equal(4.8, calculator.run('24 / 5'))
    assert_equal(-17.0, calculator.run('(-85) / 5'))
  end
  ...
  def test_pharentesis
    calculator = Calculator.new
    assert_equal(10.0, calculator.run('(1 + 4) * 2'))
    assert_equal(7.0, calculator.run('((((((5 + 2))))))'))
    assert_equal(-5.0, calculator.run('(-5) + 0'))
  end

  def test_log
    calculator = Calculator.new
    assert_equal(2.0, calculator.run('log 100 10'))
